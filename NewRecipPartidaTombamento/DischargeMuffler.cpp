...
{
	POLOIO POLOIOobj("DISMUFFLER.dat");

	expTemp			 =	POLOIOobj.getParamVal("dis_muffler","experimental_temperature", "left") + 273.15;
	pressConvergCrit =	POLOIOobj.getParamVal("dis_muffler","pressure_convergence_criterion", "left");
	massConvergCrit =	POLOIOobj.getParamVal("dis_muffler","mass_convergence_criterion", "left");
	tempConvergCrit =	POLOIOobj.getParamVal("dis_muffler","temperature_convergence_criterion", "left");
	velRelaxCoef =		POLOIOobj.getParamVal("dis_muffler","velocity_relaxation_coefficient", "left");
	pressRelaxCoef =	POLOIOobj.getParamVal("dis_muffler","pressure_relaxation_coefficient", "left");
	tempRelaxCoef =		POLOIOobj.getParamVal("dis_muffler","temperature_relaxation_coefficient", "left");
	zeroFlFricFact =	POLOIOobj.getParamVal("dis_muffler","zero_flow_friction_factor", "left");
	chambNum =			POLOIOobj.getParamVal("dis_muffler","chamber_number", "left");
	entTemp =			POLOIOobj.getParamVal("dis_muffler","entrance_temperature", "left");
...
void DischargeMuffler::dimenVec()
{
	p.resize(volNum+2, 0);
	T.resize(volNum+2, 0);
	u.resize(volNum+2, 0);
...
void DischargeMuffler::simul(double massFl, double disChambPress, double disChambTemp, double compChambTemp, double extTemp, double disLinePress, double disLineTemp)
{
	tempResid = 1;
	massResid = 1;
	pressResid = 1;
	...
	if(cont == 0)
	{
		...
		refrig.setTemp(disChambTemp); 
		refrig.setPress(disChambPress);
		...
		boundCond(disLinePress,disLineTemp,extTemp);
		inicVect(disChambTemp - 273.15,  disLineTemp - 273.15);
	}
...
void DischargeMuffler::readGeomDat()
{
	POLOIO POLOIOobj("DISMUFFLER.dat");
	...
	for(id = 0; id < chambNum; id++)
	{
		...
		Lcam[id] = POLOIOobj.getParamVal("dis_muffler","chamber_length",idStr, "left"); 
		Dcam[id] = POLOIOobj.getParamVal("dis_muffler","chamber_diameter",idStr, "left");
	...
	for(int i = 0; i < tubesNumber; i++)
	{
		...
		Ltubo[i] =		POLOIOobj.getParamVal("dis_muffler","tube_length",idStr, "left"); 
		Dtubo[i] =		POLOIOobj.getParamVal("dis_muffler","tube_diameter",idStr, "left");
		tubeVolNum[i] = POLOIOobj.getParamVal("dis_muffler","tube_volumes_number",idStr, "left");
...
void DischargeMuffler::boundCond(double disLinePress, double disLineTemp, double extTempVal)
{
		p[volNum] = disLinePress;					
		T[volNum] = disLineTemp;			
		extTemp = extTempVal;					
}

void DischargeMuffler::inicVect(double disChambTemp, double disLineTemp)
{
	double Tini;

	Tini = (disChambTemp + disLineTemp)/2 + 273.15;		
...
void DischargeMuffler::calcVel( double massFl)
{
	double leff,fm,Cc;
	double Di,So,Si;
	double ze, zw;
	...
	u[volNum] = (u[volNum-1]*rou[volNum-1]- (ro[volNum] - ro0[volNum])*dx[volNum]/dt)/rou[volNum];
	...
	for(int i = 1; i < volNum; i++)
	{
		...
		{	
			if(Re[i] > 2300)
			{
				f[i]=0.25*pow((log10(4.13E-4/3.7+5.74/(pow(Re[i],0.9)))),(-2));

				f[i]=-2.0*log10(4.13E-4/3.7+2.51/(Re[i]*pow(f[i],0.5)));
...
void DischargeMuffler::calcPress(double massFl)
{
	int ini, fim;
	double ze, zw;
	...
	for(int i = 1; i < volNum; i++)
	{
		 ...
		 if(i == 1) 
		 {
			awp[i] = 0;
			aep[i] = mue[i]*d[i] - (0.5-ze)*Au[i]*u[i]/(R*T[i+1]);
			app[i] = mue[i]*d[i] + (0.5+ze)*Au[i]*u[i]/(R*T[i+1])+ ATp[i]*dx[i]/(R*T[i]*dt);
			sup[i] = massFl - roe[i]*Au[i]*u[i]- (ro[i] - ro0[i])*ATp[i]*dx[i]/dt;
		 }
	...
	if(roe[volNum] == 0)
	{
			cout << roe[volNum]  << endl;
	}

	u[volNum] = (u[volNum-1]*roe[volNum-1]- (ro[volNum] - ro0[volNum])*dx[volNum]/dt)/roe[volNum];
}

void DischargeMuffler::calcTemp(double massFl, double compChambTemp)
{
	int ini, fim;
	double ze, zw;
	double me, mw;
	...
	for(int i = 1; i < volNum; i++)			
	{
		...
		if(i == 1) 
		{
			apT[i] = aeT[i] + (0.5+zw)*massFl*cp + cp*M0/dt + UA;
			awT[i] = 0.0;
			suT[i] = V/dt*(p[i]-p0[i]) + M0*cp*T0[i]/dt+ UA*extTemp + (0.5+zw)*massFl*cp*compChambTemp;										
		}
...
vector<double> DischargeMuffler::TDMA(int ISTART,int IEND, vector<double> ae, vector<double> aw, vector<double> ap, vector<double> su, vector<double> PHI)
{
...
void DischargeMuffler::calcDt(double deltaTheta, double shaftVel)
{
